...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "relative bg-[#0f0f0f] text-[#e5e5e5] flex flex-col border border-neutral-800 hover:bg-white/[0.02] transition-colors",
        className
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("leading-none font-semibold text-[#e5e5e5]", className)}
      {...props}
    />
  )
}
